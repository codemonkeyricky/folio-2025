...
function remap(input: number, inLow: number, inHigh: number, outLow: number, outHigh: number): number {
    return ((input - inLow) * (outHigh - outLow)) / (inHigh - inLow) + outLow
}

function remapClamp(input: number, inLow: number, inHigh: number, outLow: number, outHigh: number): number {
...
function signedModDelta(a: number, b: number, mod: number): number {
...
function segmentCircleIntersection(x1: number, y1: number, x2: number, y2: number, cx: number, cy: number, r: number): Intersection[] {
    const dx = x2 - x1
    const dy = y2 - y1
...
const TAU = 2 * Math.PI
var mod = function (a: number, n: number) { return (a % n + n) % n } // modulo

var equivalent = function (a: number) { return mod(a + Math.PI, TAU) - Math.PI } // [-π, +π]

function smallestAngle(current: number, target: number): number {
...
function lineIntersectsCircle(p1: Point, p2: Point, center: Point, radius: number): boolean {
...
function circleIntersectsPolygon(center: Point, radius: number, poly: Point[]): boolean {
