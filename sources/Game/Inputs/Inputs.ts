...
export class Inputs
{
    ...
    private pointer: any
    private wheel: any
    private interactiveButtons: any
    private nipple: any

    constructor(actions: any[] = [], filters: any[] = [])
    {
        this.game = Game.getInstance()
        this.events = new Events()

        this.actions = new Map()
        this.filters = new ObservableSet((event: any) =>
        {
            if(event.type === 'add')
            {
    ...
    setGamepad()
    {
        ...
        this.gamepad.events.on('change', (key: any) =>
        {
            this.updateMode(Inputs.MODE_GAMEPAD)
            this.change(`Gamepad.${key.name}`, key.value)
        })

        this.gamepad.events.on('joystickChange', (joystick: any) =>
        {
            this.change(`Gamepad.joystick${joystick.name.charAt(0).toUpperCase() + joystick.name.slice(1)}`, 0)
        })
    }
    ...
    setPointer()
    {
        ...
        this.pointer.events.on('down', () =>
        {
            ...
            this.start('Pointer.any', 1)
        })

        this.pointer.events.on('up', () =>
        {
            this.updateMode(this.pointer.mode === Pointer.MODE_MOUSE ? Inputs.MODE_MOUSEKEYBOARD : Inputs.MODE_TOUCH)
            this.end('Pointer.any', 0)
        })
